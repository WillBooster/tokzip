...
describe('compress/decompress', () => {
  ...
  test('keeps stored frames and document models independent across calls', () => {
    const inputs = [JAPANESE_PROMPT, MIXED_ANSWER, 'export const answer = 42;\n'.repeat(20)];
    const frames = inputs.map((input) => compress(input));
    const copies = frames.map((frame) => [...frame]);
    for (const i of [2, 0, 1, 0, 2, 1]) {
      expect(decompress(frames[i]!)).toBe(inputs[i]!);
      expect([...compress(inputs[i]!)]).toEqual(copies[i]!);
    }
    expect(frames.map((frame) => [...frame])).toEqual(copies);
  });
