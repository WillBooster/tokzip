 ...
 *
 *   bun scripts/bench/bench.ts [--speed] [--codec-only] [--repeat <count>] [--json <out.json>]
 */
...
function main(): void {
  ...
  const methods = args.includes('--codec-only') ? [] : METHODS;
  const repeatIndex = args.indexOf('--repeat');
  const repeat = repeatIndex === -1 ? 1 : Number(args[repeatIndex + 1]);
  ...
  if (jsonIndex !== -1 && (jsonPath === undefined || jsonPath.startsWith('--'))) {
    throw new Error('usage: bun run bench [--speed] [--codec-only] [--repeat <count>] [--json <file>]');
  }
  ...
  let compressMs = 0;
  let decompressMs = 0;
  // Repeated measurements exclude first-use initialization; the default still includes it.
  if (repeat > 1) for (const doc of docs) compress(doc.content);
  for (const doc of docs) {
    let frame: Uint8Array = new Uint8Array();
    for (let round = 0; round < repeat; round++) {
      const started = performance.now();
      frame = compress(doc.content);
    ...
    add(`corpus ${doc.corpus}`, doc, sizes);
  ...
  for (const doc of docs) fingerprint.update(JSON.stringify([doc.corpus, doc.language, doc.content]));
  const report: Record<string, unknown> = {
    commit: gitCommit(),
    ...wasmInfo(),
    corpusFingerprint: fingerprint.digest('hex'),
  ...
  if (speed) {
    const speeds = {
      compressMBps: (totalMb * repeat) / (compressMs / 1000),
      decompressMBps: (totalMb * repeat) / (decompressMs / 1000),
    };
    report['speed'] = speeds;
    console.log(
  ...
  console.log(`\nround-trip: ${docs.length * repeat} tokzip frames verified; wasm module ${report['wasmBytes']} bytes`);
  if (jsonPath) {
  ...
  for (const corpusDir of corpusDirs()) {
    for (const language of readdirLanguages(corpusDir)) {
      ...
      for (const line of readFileSync(manifestPath, 'utf8').split('\n')) {
        ...
        docs.push({
          corpus: basename(dirname(corpusDir)),
          language,
          bucket: BUCKETS.find(([limit]) => bytes <= limit)![1],
          content,
          bytes,
        });
      }
...
function wasmInfo(): { wasmBytes: number; wasmSha256: string } {
  const wasm = readFileSync(join(import.meta.dir, '../../wasm/tokzip.wasm'));
  return { wasmBytes: wasm.length, wasmSha256: createHash('sha256').update(wasm).digest('hex') };
}
