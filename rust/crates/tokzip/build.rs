...
//! language's trained dictionary (its group's shared part and its suffix): palette indices
//! in `$OUT_DIR/grams` and language masks in `$OUT_DIR/gram-masks`. `lang.rs` includes
...
fn main() {
    ...
    let group_literals: Vec<Vec<u8>> = Group::ALL
        ...
        .map(|group| {
            ...
            if lz::LiteralModels::try_from_packed(&part).is_some() {
                part
            } else {
                println!(
    ...
    let models: Vec<lz::Models> = LANGUAGES
        ...
        .zip(&language_parts)
        .map(|((_, group), part)| {
            lz::LiteralModels::from_packed(&group_literals[*group as usize]).with_language(part)
        })
        .collect();
    ...
    for (i, (name, group)) in LANGUAGES.iter().enumerate() {
        ...
        let mut prefix = read_required(&root.join("dict/wrapper.bin"));
        prefix.extend_from_slice(&shared_parts[*group as usize]);
        std::fs::write(
            &dict_path,
            pack::pack_dictionary(&suffix, &models[i], &prefix),
        )
    ...
    let table = grams::gram_table(&trained_dictionaries);
    let mut masks: Vec<u32> = (0..1 << grams::GRAM_BITS)
        .map(|h| grams::gram_mask(&table, h))
        .collect();
    masks.sort_unstable();
    masks.dedup();
    assert!(masks.len() <= u16::MAX as usize + 1);
    let indices: Vec<u8> = (0..1 << grams::GRAM_BITS)
        .flat_map(|h| {
            (masks.binary_search(&grams::gram_mask(&table, h)).unwrap() as u16).to_le_bytes()
        })
        .collect();
    std::fs::write(&grams_path, indices).expect("write grams");
    let masks_path = out.join("gram-masks");
    let masks: Vec<u8> = masks
        .iter()
        .flat_map(|mask| {
            let [a, b, c, _] = mask.to_le_bytes();
            [a, b, c]
        })
        .collect();
    std::fs::write(&masks_path, masks).expect("write gram masks");
    std::fs::write(
        out.join("assets.rs"),
        format!(
            "static ASSETS: [Assets; {}] = {assets};\nstatic GROUP_PRIORS: [&[u8]; {}] = {group_priors};\nstatic GROUP_DICTS: [&[u8]; {}] = {group_dicts};\nstatic GRAM_TABLE: &[u8] = include_bytes!({grams_path:?});\nstatic GRAM_MASKS: &[u8] = include_bytes!({masks_path:?});\n",
            LANGUAGES.len(),
            Group::ALL.len(),
            Group::ALL.len()
        ),
