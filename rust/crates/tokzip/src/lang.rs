...
use crate::grams::{gram_hash, gram_mask, GRAM_BITS};
pub use crate::languages::LANGUAGE_COUNT;
use crate::languages::{Group, LANGUAGES};
use crate::lz::{decode_doc, LiteralModels, Models, Primed, Segment};
...
// `GRAM_TABLE` (16-bit indices) and `GRAM_MASKS` (three-byte language sets).
include!(concat!(env!("OUT_DIR"), "/assets.rs"));
const _: () = assert!(GRAM_TABLE.len() == (1 << GRAM_BITS) * 2);
...
fn language_models(lang: u8) -> Models {
    static LITERALS: [OnceLock<Arc<LiteralModels>>; Group::ALL.len()] =
        [const { OnceLock::new() }; Group::ALL.len()];
    let (_, group) = LANGUAGES[lang as usize];
    LITERALS[group as usize]
        .get_or_init(|| LiteralModels::from_packed(GROUP_PRIORS[group as usize]))
        .with_language(ASSETS[lang as usize].priors)
}

/// Appends a dictionary part coded with `models`, using the preceding `bytes` as its
/// dictionary (`pack.rs`); an empty `packed` is an empty part.
fn unpack_dictionary(packed: &[u8], models: &Models, mut bytes: Vec<u8>) -> Vec<u8> {
    ...
    let prefix = Primed::new(bytes.clone(), models.clone());
    let segments = [Segment {
        end: len as usize,
        lang: 0,
    }];
    decode_doc(&|_| &prefix, body, len as usize, &segments, &mut bytes)
...
/// Relative dictionary overlap in the row `row_of(position)` selects.
fn add_gram_hits(doc: &[u8], rows: &mut [[i32; LANGUAGE_COUNT]], row_of: impl Fn(usize) -> usize) {
    if doc.len() < 4 {
        return;
    }
    for pos in 0..doc.len() - 3 {
        let mut mask = detection_mask(gram_hash(doc, pos));
        let row = &mut rows[row_of(pos)];
        // Subtracting misses differs from adding hits by a constant shared by every
        // language in the window, so both the Viterbi path and global argmax are unchanged.
        let delta = if mask.count_ones() as usize > LANGUAGE_COUNT / 2 {
            mask ^= (1 << LANGUAGE_COUNT) - 1;
            -1
        } else {
            1
        };
        while mask != 0 {
            row[mask.trailing_zeros() as usize] += delta;
            mask &= mask - 1;
        }
    }
}
...
#[inline]
fn detection_mask(hash: usize) -> u32 {
    let at = hash * 2;
    let index = u16::from_le_bytes([GRAM_TABLE[at], GRAM_TABLE[at + 1]]) as usize;
    gram_mask(GRAM_MASKS, index)
}
...
mod tests {
    ...
    fn embedded_assets_match_the_committed_assets() {
        ...
        let table = crate::grams::gram_table(&dictionaries);
        for hash in 0..1 << GRAM_BITS {
            assert_eq!(detection_mask(hash), gram_mask(&table, hash));
        }
    }
