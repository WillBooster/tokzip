...
//! `lz::LiteralModels::from_packed` and `lang::primed`).
//!
...
//! range-coded body of the part coded as one segment with the given models and prefix dictionary.
...
/// A dictionary part coded with `models` against its preceding `prefix`.
pub fn pack_dictionary(part: &[u8], models: &Models, prefix: &[u8]) -> Vec<u8> {
    let primed = Primed::new(prefix.to_vec(), models.clone());
