...
//! table the build script precomputes for it (`build.rs`). The raw table holds a three-byte
//! language set per hash; the module embeds a 16-bit index per hash and a palette of unique
//! sets. Detection reads that palette without initializing any dictionaries.
