...
// ---------------------------------------------------------------------------
// Probability models (logical flat layout; the layout is format identity)
// ---------------------------------------------------------------------------
...
pub struct Models {
    own: Vec<u16>,
    literals: Arc<LiteralModels>,
}

pub struct LiteralModels {
    classes: LitClasses,
    probs: Vec<u16>,
    /// One-based offsets; zero means a tree left flat by training.
    tree_at: Vec<u32>,
}
...
impl Models {
    ...
    #[cfg(any(test, feature = "train"))]
    fn dense(lit_classes: LitClasses, mut probs: Vec<u16>) -> Self {
        let literal_probs = probs.split_off(LIT);
        probs.shrink_to_fit();
        Self {
            own: probs,
            literals: Arc::new(LiteralModels {
                classes: lit_classes,
                probs: literal_probs,
                tree_at: (0..TREES).map(|t| (t * 256 + 1) as u32).collect(),
            }),
        }
    }
    ...
    pub fn dense_probs(&self) -> Vec<u16> {
        ...
        probs[..LIT].copy_from_slice(&self.own);
        for t in 0..TREES {
            let first = LIT + t * 256;
            if let Some(at) = self.tree_start(first) {
                probs[first..first + 256].copy_from_slice(&self.literals.probs[at..at + 256]);
            }
        }
        probs
    }
    ...
    fn tree_start(&self, first: usize) -> Option<usize> {
        let at = self.literals.tree_at[(first - LIT) >> 8];
        at.checked_sub(1).map(|at| at as usize)
    }
...
impl LiteralModels {
    /// Restores a group's packed literal trees (`pack.rs`): class tables followed by
    /// 256-node trees encoded as subtree flags and non-default probability values.
    pub fn from_packed(literal: &[u8]) -> Arc<Self> {
        Self::try_from_packed(literal).expect("packed literal priors match the model layout")
    }

    /// Combines these shared literal priors with the language-specific nodes.
    pub fn with_language(self: &Arc<Self>, language: &[u8]) -> Models {
        assert_eq!(language.len(), LIT);
        Models {
            own: language.iter().map(|&q| unquantize(q)).collect(),
            literals: Arc::clone(self),
        }
    }

    /// Restores a group's packed literal trees, rejecting stale layouts.
    pub fn try_from_packed(literal: &[u8]) -> Option<Arc<Self>> {
        if literal.len() < 512 {
            return None;
        }
        let mut lit_classes = ([0u8; 256], [0u8; 256]);
        ...
        for (t, slot) in tree_at.iter_mut().enumerate() {
            ...
            *slot = probs.len() as u32 + 1;
            probs.extend_from_slice(&tree);
        }
        rest.is_empty().then_some(Arc::new(Self {
            classes: lit_classes,
            probs,
            tree_at,
        }))
...
/// Marks a tree absent from `LiteralModels::tree_at` (flat) or `DocModels::slot` (not used yet).
const UNUSED: u32 = 0;
...
impl<'p> DocModels<'p> {
    fn new(base: &'p Models) -> Self {
        ...
        probs.clear();
        probs.extend_from_slice(&base.own);
        slot.fill(UNUSED);
        Self {
            base,
            classes: &base.literals.classes,
            probs,
            slot,
        }
    }
    ...
    fn lit_block(&mut self, prev: (u8, u8)) -> usize {
        self.tree(lit_block(self.classes, prev), 256)
    }
    ...
    fn tree(&mut self, first: usize, len: usize) -> usize {
        ...
        if self.slot[t] == UNUSED {
            ...
            match self.base.tree_start(first) {
                Some(at) => self
                    .probs
                    .extend_from_slice(&self.base.literals.probs[at..at + len]),
    ...
    fn into_models(self) -> Models {
        ...
        Models::dense(*self.classes, probs)
    }
}
...
/// Depth-first walk over one packed tree's flag bits (see `LiteralModels::from_packed`): records the
...
fn len_prices(probs: &[u16], base: usize) -> [u32; MATCH_MAX + 1] {
    let mut prices = [0; MATCH_MAX + 1];
    tree_prices(
        probs,
        base + 2,
        price_bit(probs[base], 0),
        &mut prices[2..10],
    );
    let high = price_bit(probs[base], 1);
    tree_prices(
        probs,
        base + 10,
        high + price_bit(probs[base + 1], 0),
        &mut prices[10..18],
    );
    tree_prices(
        probs,
        base + 18,
        high + price_bit(probs[base + 1], 1),
        &mut prices[18..],
    );
    prices
}

/// Prices every leaf by visiting each internal node once. Descending iteration keeps the
/// current level intact while its children overwrite the same buffer.
fn tree_prices(probs: &[u16], base: usize, prefix: u32, prices: &mut [u32]) {
    prices[0] = prefix;
    let mut width = 1;
    while width < prices.len() {
        for i in (0..width).rev() {
            let price = prices[i];
            let prob = probs[base + width + i];
            prices[2 * i] = price + price_bit(prob, 0);
            prices[2 * i + 1] = price + price_bit(prob, 1);
        }
        width *= 2;
    }
}
...
static PROB_PRICES: [u8; 2048] = build_prob_prices();

/// Rounded 1/16-bit prices for each 11-bit probability, computed with fixed-point log2
/// so every build host produces the same table without floating-point library calls.
const fn build_prob_prices() -> [u8; 2048] {
    let mut prices = [0; 2048];
    prices[0] = 176;
    let mut p = 1u32;
    while p < 2048 {
        let exponent = 31 - p.leading_zeros();
        let mut value = (p as u64) << (31 - exponent);
        let mut fraction = 0;
        let mut bit = 0;
        while bit < 16 {
        ...
        prices[p as usize] = (((11 - exponent) * 65536 - fraction + 2048) >> 12) as u8;
        p += 1;
    }
    prices
}
...
fn price_bit(prob: u16, bit: u32) -> u32 {
    ...
    u32::from(PROB_PRICES[p as usize])
}
...
impl DistPrices {
    fn new(models: &DocModels, group: usize) -> Self {
        ...
        for (ls, row) in slot.iter_mut().enumerate() {
            tree_prices(&models.probs, slot_base + ls * 64, 0, row);
        }
...
/// Length of the common prefix, comparing full SIMD lanes on wasm.
#[inline]
fn common_prefix(a: &[u8], b: &[u8]) -> usize {
    #[cfg(target_arch = "wasm32")]
    {
        // SAFETY: the supported wasm runtimes implement SIMD; the helper bounds every load.
        unsafe { common_prefix_simd(a, b) }
    }
...
unsafe fn common_prefix_simd(a: &[u8], b: &[u8]) -> usize {
    use std::arch::wasm32::{i8x16_bitmask, i8x16_ne, v128_load};
    let n = a.len().min(b.len());
    let mut i = 0;
    while i + 16 <= n {
        // SAFETY: both slices contain the full 16-byte lane.
        let mask = unsafe {
            i8x16_bitmask(i8x16_ne(
...
impl StatePrices {
    fn new(models: &DocModels, state: usize) -> Self {
        let probs = &models.probs;
        let mat = price_bit(probs[IS_MATCH + state], 1);
        let rep = mat + price_bit(probs[IS_REP + state], 1);
        let explicit = mat + price_bit(probs[IS_REP + state], 0);
        Self {
            literal: price_bit(probs[IS_MATCH + state], 0),
            short_rep: rep
                + price_bit(probs[IS_REP_G0 + state], 0)
                + price_bit(probs[IS_REP0_LONG + state], 0),
            reps: std::array::from_fn(|idx| rep + models.price_rep_choice(state, idx)),
            history: explicit + price_bit(probs[IS_DICT + state], 0),
            dictionary: explicit + price_bit(probs[IS_DICT + state], 1),
        }
    }
}
...
fn run_encode_optimal(
    ...
    while pos < end {
        ...
        let match_len_price = len_prices(&models.probs, LEN);
        let rep_len_price = len_prices(&models.probs, REP_LEN);
        let dict_len_price = len_prices(&models.probs, DICT_LEN);
        let state_prices: [StatePrices; NUM_STATES] =
            std::array::from_fn(|state| StatePrices::new(models, state));
        let dlen = mf.dict.bytes.len();
        let hist_prices = DistPrices::new(models, HIST_DIST);
        let dict_prices = DistPrices::new(models, DICT_OFF);

        // Inside a sufficiently long match, limit parse effort to literals and reps instead
        // of searching for more explicit matches at every position.
        ...
        for i in 0..(end_target - pos) {
            ...
            for idx in 0..4usize {
                ...
                if idx == 0 && l >= 1 {
                    let price = base + state_prices[state].short_rep;
                ...
                let choice = base + state_prices[state].reps[idx];
            ...
            // A sufficiently long rep avoids an additional chain walk.
            ...
            let hist_bit = state_prices[state].history;
            let dict_bit = state_prices[state].dictionary;
...
mod tests {
    ...
    fn packed_priors_restore_the_raw_model() {
        ...
        let restored = LiteralModels::from_packed(&crate::pack::literal_part(&raw))
            .with_language(&crate::pack::language_part(&raw));
        assert_eq!(restored.literals.classes, expected.literals.classes);
